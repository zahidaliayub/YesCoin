...
public:
    ...
    int64_t GetYesDebit(const CTxIn& txin) const;
    int64_t GetYesCredit(const CTxOut& txout) const;
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nYES, int64_t& nYes) const
    {
        nYES = 0;
        nYes = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nYes += GetYesCredit(txout);
            } else
                nYES += GetCredit(txout);
            if (!MoneyRange(nYES)
                || !MoneyRange(nYes))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
...
public:
    ...
    mutable bool fAvailableYesCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableYesCreditCached;
    
    mutable int64_t nCredYESCached;
    mutable int64_t nCredYesCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableYesCreditCached = false;
    ...
    bool GetCredit(int64_t& nCredYES, int64_t& nCredYes, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredYESCached = 0;
            nCredYesCached = 0;
            pwallet->GetCredit(*this, nCredYESCached, nCredYesCached);
            fCreditSplitCached = true;
        };
        
        nCredYES = nCredYESCached;
        nCredYes = nCredYesCached;
        return true;
    }
    ...
    int64_t GetAvailableYesCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableYesCreditCached)
            return nAvailableYesCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableYesCredit() : value out of range");
            };
        };
        
        nAvailableYesCreditCached = nCredit;
        fAvailableYesCreditCached = true;
        return nCredit;
    };
