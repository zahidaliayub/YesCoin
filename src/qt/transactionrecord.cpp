...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return YesGUI::tr("Received with");
    case RecvFromOther:
        return YesGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return YesGUI::tr("Sent to");
    case SendToSelf:
        return YesGUI::tr("Payment to yourself");
    case Generated:
        return YesGUI::tr("Mined");
    case RecvYes:
        return YesGUI::tr("Received yes");
    case SendYes:
        return YesGUI::tr("Sent yes");
    case Other:
        return YesGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvYes:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendYes:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredYES, nCredYes;
    wtx.GetCredit(nCredYES, nCredYes, true);
    int64_t nCredit = nCredYES + nCredYes;
    ...
    {
        if (nNet > 0 && nCredYes > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvYes, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendYes, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvYes;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv yes
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendYes;
                    sub.address = CBitcoinAddress(ckidD).ToString();
