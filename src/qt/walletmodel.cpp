...
WalletModel::WalletModel(CWallet *wallet, OptionsModel *optionsModel, QObject *parent) :
    ...
    cachedBalance(0), cachedYesBal(0), cachedStake(0), cachedUnconfirmedBalance(0), cachedImmatureBalance(0),
...
qint64 WalletModel::getYesBalance() const
{
    return wallet->GetYesBalance();
}
...
void WalletModel::checkBalanceChanged()
{
    ...
    qint64 newYesBal = getYesBalance();
    qint64 newStake = getStake();
    qint64 newUnconfirmedBalance = getUnconfirmedBalance();
    qint64 newImmatureBalance = getImmatureBalance();

    if (cachedBalance != newBalance
        || cachedYesBal != newYesBal
        ...
        || cachedImmatureBalance != newImmatureBalance)
    {
        ...
        cachedUnconfirmedBalance = newUnconfirmedBalance;
        cachedImmatureBalance = newImmatureBalance;
        emit balanceChanged(newBalance, newYesBal, newStake, newUnconfirmedBalance, newImmatureBalance);
    }
}
...
WalletModel::SendCoinsReturn WalletModel::sendCoinsAnon(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
    ...
    if (vNodes.empty())
        return SendCoinsReturn(SCR_ErrorWithMsg, 0, QString::fromStdString("YEScoin is not connected!"));
    ...
    {
        nBalance = wallet->GetYesBalance();
        if ((nTotalOut + MIN_TX_FEE_ANON) > nBalance)
            return SendCoinsReturn(SCR_AmountWithFeeExceedsYesBalance, MIN_TX_FEE_ANON);
    };

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            if (rcp.txnTypeInd == TXT_YES_TO_YES
                || rcp.txnTypeInd == TXT_ANON_TO_YES)
            {
                // -- out sdc
                std::string sError;
