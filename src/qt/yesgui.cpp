...
YesGUI::YesGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new YesBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("YesClient") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/yes"));
    setWindowIcon(QIcon(":icons/yes"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/yes/index.html");
#else
    QFile html("/opt/yes/index.html");
#endif
...
void YesGUI::pageLoaded(bool ok)
{
...
void YesGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);

}

void YesGUI::urlClicked(const QUrl & link)
{
...
void YesGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/yes"), tr("&About YEScoin"), this);
    aboutAction->setToolTip(tr("Show information about YEScoin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for YEScoin"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/yes"), tr("&Show / Hide"), this);
...
void YesGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("YesClient") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/yes_testnet"));
            setWindowIcon(QIcon(":icons/yes_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/yes_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("YesClient client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/yes_testnet"));
...
void YesGUI::setWalletModel(WalletModel *walletModel)
{
...
void YesGUI::setMessageModel(MessageModel *messageModel)
{
...
void YesGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("YesClient client"));
    trayIcon->setIcon(QIcon(":/icons/yes"));
...
#ifndef Q_OS_MAC
void YesGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void YesGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to YEScoin network", "", count));
}

void YesGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void YesGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void YesGUI::changeEvent(QEvent *e)
{
...
void YesGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void YesGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::YES, nFeeRequired));
...
void YesGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void YesGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void YesGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void YesGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void YesGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void YesGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid YEScoin address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void YesGUI::handleURI(QString strURI)
{
...
void YesGUI::setEncryptionStatus(int status)
{
...
void YesGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void YesGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void YesGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void YesGUI::updateWeight()
{
    if (!pwalletMain)
        return;
