...
#include "yesbridge.moc"

YesBridge::YesBridge(YesGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
// This is just a hook, we won't really be setting the model...
void YesBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void YesBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void YesBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void YesBridge::populateOptions()
{
...
// Transactions
bool YesBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void YesBridge::clearRecipients()
{
    recipients.clear();
}

bool YesBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    {
        int inputType; // 0 YES, 1 Yes
        switch(rcp.txnTypeInd)
        {
            case TXT_YES_TO_YES:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::YES, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_YES_TO_ANON:
                formatted.append(tr("<b>%1</b> to YES %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::YES, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> YES, ring size %2 to YES %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::YES, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_YES:
                formatted.append(tr("<b>%1</b> YES, ring size %2 to YES %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::YES, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::YES, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsYesBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your yes balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::YES, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void YesBridge::openCoinControl()
{
...
void YesBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void YesBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap YesBridge::listAnonOutputs()
{
...
void YesBridge::populateTransactionTable()
{
...
void YesBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void YesBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString YesBridge::transactionDetails(QString txid)
{
...
// Addresses
void YesBridge::populateAddressTable()
{
...
void YesBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void YesBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString YesBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString YesBridge::lastAddressError()
{
    QString sError;
...
QString YesBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void YesBridge::updateAddressLabel(QString address, QString label)
{
...
bool YesBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool YesBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void YesBridge::appendMessages(QVariantList messages, bool reset)
{
    emitMessages(messages, reset);
}

void YesBridge::appendMessage(int row)
{
...
void YesBridge::populateMessageTable()
{
...
void YesBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool YesBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool YesBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString YesBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);
}

QString YesBridge::addressForPubKey(QString pubkey)
{
    return addressModel->atm->addressForPubkey(pubkey);
}

bool YesBridge::setPubKey(QString address, QString pubkey)
{
...
bool YesBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
...
QString YesBridge::createGroupChat(QString label)
{
...
QString YesBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList YesBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
QString YesBridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap YesBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap YesBridge::listLatestBlocks()
{
...
QVariantMap YesBridge::findBlock(QString searchID)
{
...
QVariantMap YesBridge::blockDetails(QString blkHash)
{
...
QVariantMap YesBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap YesBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap YesBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap YesBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap YesBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap YesBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap YesBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap YesBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap YesBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with Yescoin prefix.");
            return result;
        }
...
QVariantMap YesBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap YesBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap YesBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
