...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog, YesBridge *bridge)
{
    if (!model) return;
    ...
    } else
    {
        ...
        int nDisplayUnit = BitcoinUnits::YES;
        ...
        l8->setStyleSheet((nChange > 0 && nChange < CENT) ? "color:red;" : ""); // Change < 0.01YES
