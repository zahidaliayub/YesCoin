...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("yesbalance", ValueFromAmount(pwalletMain->GetYesBalance())));
    ...
    if (nNodeMode == NT_FULL)
    {
        ...
        obj.push_back(Pair("yessupply", ValueFromAmount(pindexBest->nAnonSupply)));
    }
...
Value getnewextaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "Returns a new YEScoin ext address for receiving payments."
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw std::runtime_error(
            "sendtoaddress <yescoinaddress> <amount> [comment] [comment-to] [narration]\n" // Exchanges use the comments internally...
            "sendtoaddress <yescoinaddress> <amount> [narration]\n"
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw std::runtime_error(
            "sendfrom <fromaccount> <toyescoinaddress> <amount> [minconf=1] [comment] [comment-to] [narration] \n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid YEScoin address: ")+s.name_);
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw std::runtime_error(
            "validatepubkey <yescoinpubkey>\n"
            "Return information about <yescoinpubkey>.");
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "Returns a new YEScoin stealth address for receiving payments anonymously."
            + HelpRequiringPassphrase());
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    if (!sxAddr.SetEncoded(sEncoded))
        throw std::runtime_error("Invalid YEScoin stealth address.");
